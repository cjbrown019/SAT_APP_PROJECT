...
namespace SAT_APP_PROJECT.MVC.UI.Utilities
{
    public class ImageUtility
    {
        ...
        public static void Delete(string path, string fileName)
        {
            ...
            //Check if designated file exists AND IS NOT THE DEFAULT -- and, if so, delete it
            if (baseFile.Exists && fileName != "noimage.jpg")
            {
                baseFile.Delete();
            }

            //Check if a thumbnail version exists for the file AND IS NOT THE DEFAULT -- and, if so, delete it
            if (thumbImg.Exists && fileName != "t_noimage.jpg")
            {
                thumbImg.Delete();
            }
        }
